...
#if 1
        /* check landmark in sight by mesh normal : very like the visual distance */
...
bool createInitMap(Frame &frame_last, Frame &frame_curr, LoaclMap &map, std::vector<FMatch> &matches)
{
    ...
    for (size_t n = 0; n < matches.size(); n++)
    {
        ...
        cv::Mat p3d_c1;
        TwoViewGeometry::Triangulate(point_last[n], point_curr[n], P1, P2, p3d_c1); //点P在p1中的特征点，p2中的特征点，P1的位姿，P2的位姿，待求的P点//
...
int main()
{
    ...
    createLandmarksFromMesh(mesh, landmarks, normals);             //copy landmarks and normals from mesh
    ...
    cv::Mat cv_K = (cv::Mat_<float>(3, 3) << 480, 0, 320, 0, 480, 240, 0, 0, 1);
    ...
    LoaclMap map(landmarks.size());                    //std::vector<Eigen::Vector3d> mpts_ size=N landmarks.size()//
    for (size_t i = 1; i < pose_num; i++)
    {
        /* get features of current frame */
        Frame &frame_last = frames.back();
        Frame frame_curr(i, landmarks.size(), K);    //construct idx_(idx), N_(N), K_(K)//
        detectFeatures(v_Twc[i], K, landmarks, normals, frame_curr.fts_, frame_curr.fts_obvs_);  //fts_ 2D{u,v}, fts_obvs_=idx//
        ...
        else
        {
            ...
            cv::Mat P1(3,4,CV_32F,cv::Scalar(0));
   			...
   			cv_K.copyTo(P1.rowRange(0,3).colRange(0,3));
   			cv::Mat P2(3,4,CV_32F);
   			cv::Mat R21(3,3,CV_32FC1);
   			cv::Mat t21(3,1,CV_32FC1);
   			
   			
			
   			Eigen::Matrix3d Rtemp= frame_curr.Twc_.block(0,0,3,3).transpose()*frame_last.Twc_.block(0,0,3,3);
   			Eigen::Vector3d ttemp= frame_curr.Twc_.block(0,0,3,3).transpose()*(frame_last.Twc_.block(0,3,3,1)-frame_curr.Twc_.block(0,3,3,1));
   			cv::eigen2cv(Rtemp, R21);
   			cv::eigen2cv(ttemp, t21);
    		R21.copyTo(P2.rowRange(0,3).colRange(0,3));
    		t21.copyTo(P2.rowRange(0,3).col(3));
    		P2 = cv_K*P2;
            ...
            for (size_t n = 0; n < matches.size(); n++)
            {
				...
				cv::Point2f point_last,point_curr;
        		point_curr = cv::Point2f(frame_curr.fts_[idx_curr][0], frame_curr.fts_[idx_curr][1]);
        		point_last = cv::Point2f(frame_last.fts_[idx_last][0], frame_last.fts_[idx_last][1]);
    			
                
        		TwoViewGeometry::Triangulate(point_last, point_curr, P1, P2, p3d_c1);
