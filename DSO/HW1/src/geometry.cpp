...
void hw::GeometryUndistorter::makeOptimalK_crop()
{
    ...
    std::vector<Eigen::Vector2f> temp(100000);

    
    for(int x=0; x<100000;x++)
    {temp[x][0] = (x-50000.0f) / 10000.0f; temp[x][1] = 0;} 
    distortCoordinates(temp);
    for(int x=0; x<temp.size();x++)
    {
        if(temp[x][0] > 0 && temp[x][0]< w_-1)
        {
    ...
    {temp[y][1] = (y-50000.0f) / 10000.0f; temp[y][1] = 0;}
    distortCoordinates(temp);
    for(int y=0; y<100000;y++)
    {
        if(temp[y][1] > 0 && temp[y][1] < h_-1)
        {
    ...
    // TODO: read DSO source code, and find out way to calculate new K
	 ...
	 K_rect_(0,2) = -minX*K_(0,0);
	 K_rect_(1,2) = -minY*K_(1,1);
