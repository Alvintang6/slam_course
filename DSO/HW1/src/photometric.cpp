...
void hw::PhotometricUndistorter::processFrame(cv::Mat& image)
{
    ...
    std::cout<<"type of"<<image.type()<<std::endl;
	int wh = w_img*h_img; 
    ...
    for(int i=0; i<wh;i++)
	{
			...
			//image.at<float>(i) = G_[image.at<float>(i)]; // 去掉响应函数
			
			image.at<char>(i) = G_[image.at<char>(i)];
	...
	for(int i=1; i<wh;i++){
	 	image.at<char>(i) *= vignette_inv_.at<float>(i);
