...
int main()
{
    ...
    for (size_t i = 1; i < pose_num; i++)
    {
        ...
        if (!init_flag)
        {
            ...
            initial_position = T_curr_last.block(0,3,3,1);
            double t_scale = 1.0;
            {
        ...
        else
        {
            ...
            cv::solvePnPRansac(obj_pts, img_pts, cv_K, cv::Mat(), r_cv, t_cv, false, cv::SOLVEPNP_ );
            //cv::solvePnPRansac(obj_pts, img_pts, cv_K, cv::Mat(), r_cv, t_cv, false, iterationsCount, reprojectionError, confidence, cv::noArray(), cv::SOLVEPNP_ITERATIVE ); 
            //solvePnP ( obj_pts, img_pts, cv_K, cv::Mat(), r_cv, t_cv, false );
          ...
          Twc_curr = Eigen::Matrix4d::Identity();
            Twc_curr.block(0,0,3,3) = R_eig.transpose();
            Twc_curr.block(0,3,3,1) = -(R_eig.transpose()*t_eig);
    ...
    std::cout<<"time used for computing: "<<time2<<std::endl;
